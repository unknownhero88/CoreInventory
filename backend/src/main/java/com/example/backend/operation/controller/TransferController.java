...
import com.example.backend.operation.dto.TransferRequest;
...
public class TransferController {
    ...
    public ResponseEntity<List<OperationResponse>> getAll(
        ...
        return ResponseEntity.ok(operationService.getAll(OperationType.TRANSFER, status));
    }
    ...
    @PostMapping
    public ResponseEntity<OperationResponse> create(@Valid @RequestBody TransferRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED).body(operationService.createTransfer(request));
    }

    @PostMapping("/{id}/validate")
    public ResponseEntity<OperationResponse> validate(@PathVariable Long id) {
        return ResponseEntity.ok(operationService.validateTransfer(id));
    }
